...
class Operacion(BaseModel):
	...
	def pagos_capital(self, fecha=date.today()):
		...
		return Operacion.objects.filter(vinculo=self, cuenta=self.cuenta, fecha__lte=fecha).exclude(vinculos__cuenta__in=cuentas_intereses).order_by('fecha')
	...
	def pagos_interes(self, fecha=date.today()):
		...
		"""
		cuentas_intereses = Cuenta.all_objects.filter(comunidad=self.comunidad, taxon__nombre="interes_predeterminado")
		return Operacion.objects.filter(vinculo=self, cuenta=self.cuenta, vinculos__cuenta__in=cuentas_intereses, fecha__lte=fecha).order_by('fecha')		
